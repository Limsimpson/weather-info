  ...
  render() {
    const sidoWrap = Object.keys(this.props.data).map((sido, idx) => {
      return (
        <li
          ...
          className={sido}
          onClick={() => {
            this.props.setSido(sido);
          }}
        >
          {sidoEngToKor[sido]}
          <span className={`sidoValue ${this.props.data[sido].rank}`}>
            {this.props.data[sido].value}
          </span>
    ...
    const infoWrap = Object.keys(this.props.data).map((sido, idx) => {
      ...
      return (
        <li
          key={idx}
          className={`infoBox${rank} ${sido === this.props.sido ? "On" : ""}`}
        >
    ...
    return (
      <div className="wrap clearfix">
        ...
        <div className="infoWrap">
          <ul className="infoBox">{infoWrap}</ul>
        </div>
      </div>
    );
  }
